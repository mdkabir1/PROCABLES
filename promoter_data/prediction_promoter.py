...
def prediction_promoter(input_data,save_model):
    ...
    def CKSNAP(fastas, gap, **kw):
       ...
       if get_min_sequence_length(fastas) < gap + 2:
           print('Error: all the sequence length should be larger than the (gap value) + 2 = ' + str(
               gap + 2) + '\n\n')
           return 0
    ...
    with open('Train_2Unsuper','ab') as f:
            f.write(open('Train_2Unsuper','rb').read())
 ...
 # Define a function to count sequences in a FASTA file
    ...
    CKSNAP_feat = CKSNAP(fastas, 4, **kw)
    PseEIIP_feat = PseEIIP(fastas, **kw)
    [s, f]=np.shape(PseEIIP_feat)
    ...
    featur=np.hstack((CKSNAP_feat,PseEIIP_feat,w2vec,PSTN_K1,PSTN_K2))
